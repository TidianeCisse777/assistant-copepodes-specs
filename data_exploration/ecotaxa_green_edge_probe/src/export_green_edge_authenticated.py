...
PROJECT_SLUG = "green_edge_2015_icecamp"
...
class RequestBudget:
    def __init__(self, limit: int) -> None:
        ...
        self.session.headers.update(
            {
                "User-Agent": "ecotaxa-green-edge-auth-export/0.1",
...
def download_job_file(budget: RequestBudget, job_id: int) -> Path:
    ...
    content_type = response.headers.get("content-type", "").lower()
    base_name = f"ecotaxa_{PROJECT_SLUG}_{PROJECT_ID}_export_job_{job_id}"
    if response.content[:4] == b"PK\x03\x04" or "zip" in content_type:
        path = RAW / f"{base_name}.zip"
    elif "tab-separated" in content_type:
        path = RAW / f"{base_name}.tsv"
    elif "csv" in content_type:
        path = RAW / f"{base_name}.csv"
    else:
        path = RAW / f"{base_name}.bin"
    write_bytes(path, response.content)
    return path
...
def inspect_if_zip(path: Path) -> None:
    if path.suffix.lower() == ".zip":
        subprocess.run([sys.executable, str(ROOT / "src" / "inspect_green_edge_export.py"), str(path)], check=False)


def write_auth_report(job_id: int, job: dict[str, Any], file_path: Path, request_count: int) -> None:
    lines = [
        f"EcoTaxa authenticated export - project {PROJECT_ID}",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=f"Authenticated EcoTaxa export for {PROJECT_LABEL} project {PROJECT_ID}."
    )
