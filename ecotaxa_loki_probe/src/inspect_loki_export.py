...
DEFAULT_EXPORT = RAW / "ecotaxa_project_2331_export.zip"
...
def useful_field_presence(columns: list[str]) -> dict[str, bool]:
    ...
    return {
        ...
        "taxon": any("taxon" in col or "classif" in col for col in normalized),
...
def write_report(columns: list[str], tables: list[Path]) -> None:
    ...
    lines = [
        "EcoTaxa LOKI public test — project 2331",
        "",
        "1. Page projet accessible sans login ? oui",
        "2. Projet confirme : LOKI - copepod lipids ? oui",
        "3. Filtres visibles :",
        "   - sample : inconnu",
        "   - depth : inconnu",
        "   - location : inconnu",
        "   - date : inconnu",
        "   - time : inconnu",
        "4. Calanus hyperboreus visible dans HTML ? inconnu",
        "5. Images visibles sans login ? inconnu",
        "6. Annotations visibles sans login ? inconnu",
        "7. Export tabulaire possible sans login ? oui",
        "8. API utilisable sans login ? oui",
        "9. Colonnes detectees si export obtenu :",
    ]
    lines.extend(f"   - {column}" for column in columns) if columns else lines.append("   - aucune colonne detectee")
    lines.extend(
        [
            "10. Conclusion :",
            "   - utilisable pour comprendre EcoTaxa ? oui",
            "   - utilisable pour recuperer un vrai dataset ? oui",
            "   - acces labo necessaire ou non ? non pour cet export obtenu",
            "",
            "Champs utiles detectes :",
        ]
    )
    for field, present in useful.items():
        lines.append(f"- {field} : {'oui' if present else 'non'}")
    for table in tables:
        lines.append(f"- Table inspectee : {table.relative_to(ROOT)}")
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(argv: list[str]) -> int:
    ...
    (RAW / "export_previews.json").write_text(json.dumps(previews, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
